...
def get_auth_dict(request,userType):
    auth_dict = {}
    user_authDict = {}
    if(userType == 'teacher_login'):
        try:
            designObj = user_designation_master.objects.get(designation_name = 'teacher')
    ...
    elif(userType == 'class_teacher_login'):
        try:
            designObj = user_designation_master.objects.get(designation_name = 'class teacher')
            userAuthObj = assign_authority.objects.get(designation_fk = designObj)
            user_authDict = json.loads(userAuthObj.authority_fk)
        except:
            auth_dict = {}
