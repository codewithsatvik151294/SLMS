...
function edit_schoolAdmin(){
    ...
    if(schoolAdmin_name_flag == true || city_flag == true || schoolAdmin_dob_flag == true || schoolAdmin_gender_flag == true || schoolAdmin_fathers_name_flag == true || schoolAdmin_email_flag == true || schoolAdmin_contact_flag == true || pincode_flag == true || branchCode_flag == true || image_flag == true){
        ...
        $.ajax({
            type: 'POST',
            url: "/manage/edit-school-admin/"+res1,
            ...
            success: function (response) {
                ...
                if(response['message'] == 'success'){
                    Swal.fire({
                        icon: 'success',
                        title: 'School Admin updated successfully.',
                    ...
                    alert('An Error occured while updating School Admin. Please try again!');
                    return false;
                }
            }
        });
...
function changeStatus(thisTxt) {
    var schoolAdminId = $(thisTxt).attr('schoolAdminId');
    const csrftoken = getCookie('csrftoken');
    if ($(thisTxt).is(':checked')) {
        ...
        $.ajax({
            type: 'POST',
            headers: { 'X-CSRFToken': csrftoken },
            url: "change-school-admin-status",
            data: { 'schoolAdminId': schoolAdminId, 'data': true },
            success: function (response) {
                if(response['message'] == 'success'){
                Swal.fire({
                    icon: 'success',
                    title: 'SchooAdmin enabled successfully.',
                    showConfirmButton: false,
                    timer: 1000
                })
            }else{
        ...
        $.ajax({
            ...
            headers: { 'X-CSRFToken': csrftoken },
            url: "change-school-admin-status",
            data: { 'schoolAdminId': schoolAdminId, 'data': false },
            success: function (response) {
                if(response['message'] == 'success'){
                Swal.fire({
                    icon: 'success',
                    title: 'School Admin disabled successfully.',
                    showConfirmButton: false,
                    timer: 1000
                })
            }else{
                alert('An Error occured while updating schoolAdmin. Please try again!');
                return false;
            }
            }
        });
