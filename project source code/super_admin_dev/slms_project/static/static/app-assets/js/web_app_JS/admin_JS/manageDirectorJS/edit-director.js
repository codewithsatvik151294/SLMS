...
function edit_director(){
    ...
    if(director_name_flag == true || director_dob_flag == true || city_flag == true || director_gender_flag == true || director_fathers_name_flag == true || director_email_flag == true || director_contact_flag == true || pincode_flag == true || branchCode_flag == true || image_flag == true){
        return false;
    }else{
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(response['message'] == 'success'){
                    ...
                    alert('An Error occured while updating director. Please try again!');
                    return false;
                }
            }
        });
...
function changeStatus(thisTxt) {
    var directorId = $(thisTxt).attr('directorId');
    const csrftoken = getCookie('csrftoken');
    if ($(thisTxt).is(':checked')) {
        ...
        $.ajax({
            type: 'POST',
            headers: { 'X-CSRFToken': csrftoken },
            url: "change-director-status",
            data: { 'directorId': directorId, 'data': true },
            success: function (response) {
                if(response['message'] == 'success'){
                Swal.fire({
                    icon: 'success',
                    title: 'Director enabled successfully.',
                    showConfirmButton: false,
                    timer: 1000
                })
            }else{
        ...
        $.ajax({
            ...
            headers: { 'X-CSRFToken': csrftoken },
            url: "change-director-status",
            data: { 'directorId': directorId, 'data': false },
            success: function (response) {
                if(response['message'] == 'success'){
                Swal.fire({
                    icon: 'success',
                    title: 'Director disabled successfully.',
                    showConfirmButton: false,
                    timer: 1000
                })
            }else{
